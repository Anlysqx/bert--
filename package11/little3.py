...
flags.DEFINE_integer('max_sentence_len',480,'define the max len of sentence')
...
def input_str_concat(inputList):
    ...
    t,c = inputList
    t = t[:FLAGS.max_sentence_len]
    c = c[:FLAGS.max_sentence_len]
    newStr = '%s__%s__'%(t,c)
...
with tf.Graph().as_default(),tf.Session() as sess:
    ...
    for yield_batch_data in get_data_batch():
        ...
        print('i : %s, loss : %s, start_acc : %s, end_acc : %s, total_acc : %s' % (total_steps, loss_val, start_acc_val, end_acc_val, total_acc_val))
